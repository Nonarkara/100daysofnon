...
|---|---|---|
| **Two Layers** | `/` | The invented novel / 3026 archive. Ten chapters, two layers (2026 Simulation · 3026 Archive), one chapter per view, punk brutalist reader. Built from `data/two-layers.json` via `build.py`. |
...
| **The Universe** | `/universe/` | The map of the whole — Two Layers as trunk, the biography and companions as stations. The navigable index. |
...
```
site/
  index.html              # Two Layers reader  (generated by build.py)
  book/index.html         # the biography      (main.css)
